...
let body=$response.body;if(body){switch(!0){case/pgc\/season\/app\/related\/recommend\?/.test($request.url):try{let t=JSON.parse(body);t.result?.cards?.length&&(t.result.cards=t.result.cards.filter(t=>2!=t.type)),body=JSON.stringify(t)}catch(i){console.log("bilibili recommend:"+i)}break;case/^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/skin\?/.test($request.url):try{let a=JSON.parse(body);delete a.data?.common_equip,body=JSON.stringify(a)}catch(e){console.log("bilibili skin:"+e)}break;case /^https?:\/\/app\.bilibili\.com\/x\/v2\/feed\/index\?/.test($request.url):
    try {
        ...
        for (let o of s.data.items) {
            if (o.hasOwnProperty("banner_item")) { 
                // 保留 banner_item 内容
                l.push(o);
            } else if (!o.hasOwnProperty("ad_info") && -1 === o.card_goto?.indexOf("ad") && ["small_cover_v2", "large_cover_v1", "large_cover_single_v9"].includes(o.card_type)) {
                // 过滤掉广告卡片，保留非广告内容
                l.push(o);
            }
        }
